...
def gradient_descent(X, y, rng, batch_size, lr=0.3, epochs=500, ada=False, 
                     class_weighing=False, class_weighing_power=1, focal_loss=False,
                     focal_lambda=2, focal_class_power=0.5):
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_batches + 1):
            ...
            if focal_loss: 
                alpha_B = np.power(alpha_B, focal_class_power) 
                P = np.sum(y_B * P_B, axis=1, keepdims=True) 
                S_B = ((1 - P) ** (focal_lambda - 1)) * (1 - P - focal_lambda * P * np.log(np.clip(P, 1e-12, 1 - (1e-12))))
                S_B *= alpha_B 
                confusion *= S_B 
            ...
            g_b = np.sum(confusion, axis=0)
            
            if not class_weighing:
                    g_w /= m
                    g_b /= m 
...
if __name__ == "__main__":
    ...
    losses, W, bias = gradient_descent(X_train, y_train, rng, epochs=200, batch_size=32, lr=0.3, ada=1, 
                                       class_weighing=True, class_weighing_power=0.3)
